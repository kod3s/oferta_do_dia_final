...
export const auth = {
  signUp: async (
    ...
    // 1. Cria o usuário passando o role nos metadados
    ...
    // 2. Aguarda 500ms para o trigger rodar primeiro
    await new Promise(r => setTimeout(r, 500))

    // 3. Força o role correto sobrescrevendo o que o trigger criou
    if (data.user) {
      await supabase
        .from('profiles')
        .update({ role })
...
export const markets = {
  ...
  create: async (userId: string, marketData: Partial<Market>) => {
    const { data: market, error } = await supabase
      .from('markets')
      .insert({ ...marketData, user_id: userId })
      .select()
      .single()
    if (error) throw error
    return market
  ...
  update: async (marketId: string, marketData: Partial<Market>) => {
    const { error } = await supabase
      .from('markets')
      .update(marketData)
      .eq('id', marketId)
...
export const offers = {
  ...
  create: async (marketId: string, offerData: Partial<Offer>) => {
    const { error } = await supabase
      .from('offers')
      .insert({ ...offerData, market_id: marketId })
    if (error) throw error
  },

  update: async (offerId: string, offerData: Partial<Offer>) => {
    const { error } = await supabase
      .from('offers')
      .update(offerData)
      .eq('id', offerId)
